...
function Signup() {
    ...
    function userEmailChange(event){
    ...
    function handleSubmit(event) {
        ...
        {
            ConfirmPassword == password ? SetData( Data => ({userName : userName , password : password,userEmail : Email})) : alert("The password doesn't match")
        }
        if(Data!= {}){
            navigate('/');
        }
    ...
    return ( 
        <div className="flex flex-col h-full w-full justify-center items-center mt-[20%]">
            ...
            <form onSubmit={(event) => {handleSubmit(event)}}>
                <p>Name</p>
                <input onChange={(event) => {userNameChange(event)}} placeholder="ex.john smith" type="text" className='border-2 border-black'/>
                <p>Email</p>
                <input onChange={(event) => {userEmailChange(event)}} placeholder="ex.archit@email.com" type="text" className='border-2 border-black'/>
                <p>Password</p>
                <input onChange={(event) => {passwordChange(event)}} placeholder="Password" type="password" className='border-2 border-black'/>
                <p>Confirm Password</p>
                <input onChange={(event) => {confirmPassword(event)}} placeholder="Password" type="password" className='border-2 border-black'/>
               
                <button className='bg-blue-500 text-white h-10 w-24' >Submit</button>
            </form>
            <p>I understood the <span className=" text-green-500"> terms and policy</span></p>
            <div>
                <p>Or sign in with</p>
                <div className='flex'>
